...
import de.adorsys.aspsp.xs2a.domain.pis.Xs2aCommonPaymentInitialisationRequest;
...
public class PaymentController12 implements PaymentApi {
    ...
    public ResponseEntity<?> initiatePayment(Object body, String paymentService, String paymentProduct, UUID xRequestID,
        ...
        Xs2aCommonPaymentInitialisationRequest paymentRequest = paymentModelMapper.mapToXs2aCommonPaymentInitialisationRequest(body, paymentService, paymentProduct);
        String certificate = paymentModelMapper.mapToTppSignatureCertificate(tpPSignatureCertificate);

        ResponseObject serviceResponse = xs2aPaymentService.createPayment(paymentRequest, certificate);

        return serviceResponse.hasError()
                   ? responseMapper.created(serviceResponse)
                   : responseMapper.created(ResponseObject.builder().body(paymentModelMapper.mapToPaymentInitiationResponse12(serviceResponse.getBody(), PaymentType.getByValue(paymentService).get())).build());
    }
