...
public class FundsConfirmationController implements FundsConfirmationApi {
    ...
    public ResponseEntity<?> checkAvailabilityOfFunds(ConfirmationOfFunds confirmationOfFunds, UUID xRequestID, String digest, String signature, byte[] tpPSignatureCertificate) {
        ...
        Optional<TppMessageGeneric> error = referenceValidationService.validateAccountReference(confirmationOfFunds.getAccount());
        return responseMapper.ok(
            error
                .map(e -> ResponseObject.<InlineResponse200>builder().fail(Arrays.asList(e)).build())
